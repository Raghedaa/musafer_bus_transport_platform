...
class SelectSeatController extends GetxController {
  ...
  void onInit() {
    ...
    if (selectedTrip != null) {
      print("Success: Trip data loaded in Controller: ${selectedTrip!.companyName}");
      _updatePrice();
    } else {
  ...
  void goToPayment() {
    ...
    try {
      if (selectedTrip == null) {
        Get.snackbar("Error", "Trip data is missing");
        return;
      }

      if (selectedSeats.isEmpty) {
        Get.snackbar("Notice", "Please select at least one seat");
        return;
      }
      ...
      Get.to(
        ...
        binding: BindingsBuilder(() {
          // نقوم بحقن الكنترولر هنا ليتعرف عليه الـ GetView
